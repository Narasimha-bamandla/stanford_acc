...
export default function Visualizer({
  ...
  const [quarterActiveIndex, setQuarterActiveIndex] = useState(0);
  ...
  for (let i = 0; i < numRows; i++) {
    ...
    for (let j = 0; j < numCols; j++) {
      if (j === 0) {
        ...
        cells.push(
          <td
            style={{ height: "21.5px", border: "1px solid #D5D5D4" }}
            key={`cell-${i}-${j}`}
          ></td>
        );
      }
  ...
  const handleSeasonYearDataChange = (season, year, quarter, index) => {
    setDisplayedWeeks([]);
    setWeekArray([]);
  ...
  const AlternateImageText = (event) => {
    if (event?.enrolled) {
      return "NoConflict";
    } else if (event?.preference) {
      if (event?.conflictStatus == "NoConflict") {
        return "PreferenceCourse";
      }
      if (event?.conflictStatus == "Conflict") {
        return "ConflictCourse";
      }
      if (event?.conflictStatus == "PreferenceConflict") {
        return "PreferenceConflictCourse";
      }
    }
  };
    ...
    <div className={`col-md-${expandStyle}`}>
      <div className="Visualizer" tabindex="0">
        <ClayLayout.ContainerFluid>
          <ClayLayout.Row
            justify="center"
            style={{ paddingTop: "5px", borderBottom: "1px solid #D5D5D4" }}
          >
            <ClayLayout.Col
              size={12}
              align={"center"}
              className="visualizer-header"
              style={{ padding: "0px", border: "none", height: "35px" }}
            >
              <h2
                className="text-3 font-weight-semi-bold"
          ...
          <ClayLayout.Row className="custom-gap">
            <ClayLayout.Col size={12} style={{ paddingTop: 6 }}>
              <div className="viz_cal">
                  ...
                  {newVisualizerData?.map((dayData, index) => (
                    ...
                    >
                      ...
                      {Object?.values(dayData)[0]?.map(
                        (timeSlot, slotIndex) => (
                          <>
                            <h3 className="visuallyHidden">{timeSlot.day}</h3>
                            <ul
                              key={slotIndex}
                              style={{
                                display: "flex",
                ...
                <table className="cal" aria-label="Weekly Calendar">
                  <thead>
                    <tr>
                      <th scope="col"></th>
                      <th
